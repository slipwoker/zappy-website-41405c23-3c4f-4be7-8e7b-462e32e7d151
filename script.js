document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    ...
    function getErrorContainer(field) {
      ...
      return field.parentElement
        ? field.parentElement.querySelector('.field-error, [data-field-error]')
        : null;
    }
    ...
    function validateField(field) {
      ...
      if (type === 'url') {
        try {
          ...
          showError(field, field.dataset.errorUrl || 'Please enter a valid URL.');
          return false;
        }
      }
      ...
      if (pattern) {
        ...
        if (!regex.test(value)) {
          showError(field, field.dataset.errorPattern || 'Please match the required format.');
          return false;
        }
      }
    ...
    contactForm.addEventListener('submit', function (e) {
      ...
      // If the form has data-ajax attribute, handle via fetch
      if (contactForm.dataset.ajax !== undefined) {
        e.preventDefault();
        ...
        fetch(action, {
          ...
          .then(function () {
            ...
            const successMessage = contactForm.querySelector(
              '.form-success-message, [data-success-message]'
            );
            if (successMessage) {
          ...
          .catch(function () {
            ...
            const errorMessage = contactForm.querySelector(
              '.form-error-message, [data-error-message]'
            );
            if (errorMessage) {
  ...
  function initScrollAnimations() {
    const elements = document.querySelectorAll(ANIMATE_SELECTOR);
  ...
  // Re-run for dynamically added elements (optional observer on DOM mutations)
  if ('MutationObserver' in window) {
    const mutationObserver = new MutationObserver(function (mutations) {
      ...
      if (hasNewNodes) initScrollAnimations();
    });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "Kanaf Mobile משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '41405c23-3c4f-4be7-8e7b-462e32e7d151',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '41405c23-3c4f-4be7-8e7b-462e32e7d151';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '41405c23-3c4f-4be7-8e7b-462e32e7d151';
